"""Re-certify a rendered counting dataset against its manifest (SSOT §7.6).
...
renderer changes, or on any manifest whose provenance is unclear:
    ...
    uv run python -m worlds.counting_with_distractors.verify data/manifests/smoke.jsonl
    uv run python -m worlds.counting_with_distractors.verify data/manifests/smoke.jsonl --repair

``--repair`` re-renders failing images from their manifest geometry and
re-certifies them. The manifest is the ground-truth ledger, so a failing image is
a corrupted artifact to restore — never a reason to touch a tolerance (SSOT §7.6
rule 2). Default is a read-only dry run. Exits non-zero if any image fails (or,
with --repair, still fails after restoration).
"""
...
from .renderer import CountingScene, render


def _scene_from_dict(scene_dict: dict) -> CountingScene:
    """Rebuild a CountingScene from a manifest dict (JSON lists -> tuples)."""
    d = dict(scene_dict)
    d["target_disks"] = [tuple(t) for t in d["target_disks"]]
    d["distractor_disks"] = [tuple(t) for t in d["distractor_disks"]]
    d["distractor_colors"] = list(d["distractor_colors"])
    return CountingScene(**d)
