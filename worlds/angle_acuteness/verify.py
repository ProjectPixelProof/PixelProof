"""Re-certify a rendered angle-acuteness dataset against its manifest (SSOT §7.6).
...
renderer changes, or on any manifest whose provenance is unclear:
    ...
    uv run python -m worlds.angle_acuteness.verify data/manifests/smoke.jsonl
    uv run python -m worlds.angle_acuteness.verify data/manifests/smoke.jsonl --repair
...
from .renderer import AngleScene, render


def _scene_from_dict(scene_dict: dict) -> AngleScene:
    return AngleScene(**scene_dict)
